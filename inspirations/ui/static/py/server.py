...
class SERVER:
    ...
    def start_daemon(self):
        """Just fucking run the daemon"""
        try:
            # Get the path to daemon.py
            script_dir = os.path.dirname(os.path.abspath(__file__))
            daemon_path = os.path.join(script_dir, "daemon.py")
            
            print(f"DEBUG: Launching daemon at: {daemon_path}")
            ...
            print(f"DEBUG: Daemon launched! PID: {process.pid}")
    ...
    def set_autostart(self, enable: bool):
        ...
        if enable:
            ...
            exec_command = f'"{self.APP_EXECUTABLE_PATH}" --daemon-only'
