...
class SERVER:
	def __init__(self):
		...
		self.BACKUP_FOLDERS_NAME: str = "usb"  # Temporary name for backup folder, needs to be dynamically
		...
		journal_log = f".backup_journal.log"
		self.JOURNAL_LOG_FILE = os.path.join(self.create_base_folder(), journal_log)
		metadata = f".backup_manifest.json"
		self.METADATA_FILE = os.path.join(self.create_base_folder(), metadata)
	...
	def incremental_backup_path(self):
		"""Returns the current incremental backup folder name/path."""
		return os.path.join(self.main_backup_path(), time.strftime("%d-%m-%Y"), time.strftime("%H-%M"))
	...
	def has_driver_connection(self, path):
		"""Stub for checking driver connection."""
		# In a real environment, this would check if the path is mounted/accessible.
		os.makedirs(path, exist_ok=True) # Ensure the stub target exists
		return os.path.exists(path)

    # -------------------------
    # Metadata Handling
    # -------------------------
    # instance metadata store (kept for compatibility)
	def get_metadata(self):
		"""Load metadata from JSON file."""
		# Resolve metadata path from current BACKUP_CONFIG in case it was modified after init
		meta_path = self.METADATA_FILE
		# keep attribute in sync
		try:
			self.METADATA_FILE = meta_path
		except Exception:
			pass
	...
	def save_metadata(self, metadata):
		...
		tmp = None
		try:
			# If metadata is empty and an existing metadata file is present, avoid
			# overwriting it with an empty object. This prevents accidental
			# truncation where a transient bug could wipe the persisted state.
			if (not metadata) and os.path.exists(self.METADATA_FILE):
				logging.warning(f"Refusing to overwrite existing metadata {self.METADATA_FILE} with empty metadata.")
				return False

			# If the existing metadata file already contains identical data,
			# skip creating a backup and avoid rewriting the file. This prevents
			# spurious backups on shutdown when no changes occurred.
			if os.path.exists(self.METADATA_FILE):
				try:
					with open(self.METADATA_FILE, 'r', encoding='utf-8') as vf:
						existing = json.load(vf)
					if existing == metadata:
						logging.debug(f"Metadata unchanged at {self.METADATA_FILE}; skipping write.")
						return True
				except Exception:
					# If reading fails, continue and attempt to write/backup as before
					pass

			# If an existing metadata file exists, create a timestamped backup to make recovery easier
			if os.path.exists(self.METADATA_FILE):
				try:
					bak = self.METADATA_FILE + ".bak." + time.strftime("%Y%m%d-%H%M%S")
					shutil.copy2(self.METADATA_FILE, bak)
					logging.info(f"Created metadata backup {bak}")
				except Exception as e:
					logging.warning(f"Could not create metadata backup {self.METADATA_FILE}: {e}")
			
			# Clean up old metadata backups (keep only the newest)
			try:
				backup_dir = os.path.dirname(self.METADATA_FILE)
				meta_name = os.path.basename(self.METADATA_FILE)
				all_backups = sorted([f for f in os.listdir(backup_dir) if f.startswith(meta_name + ".bak.")], reverse=True)
				keep = 3
				for i, backup in enumerate(all_backups):
					if i >= keep:
						file_path = os.path.join(backup_dir, backup)
						os.remove(file_path)
						logging.info(f"Deleted old metadata backup: {file_path}")
			except Exception as e:
				logging.warning(f"Could not clean up old metadata backups for {self.METADATA_FILE}: {e}")
				
			os.makedirs(os.path.dirname(self.METADATA_FILE), exist_ok=True)

			# write to temp file in same dir
			fd, tmp = tempfile.mkstemp(prefix=".meta_tmp_", dir=os.path.dirname(self.METADATA_FILE))
			with os.fdopen(fd, 'w', encoding='utf-8') as f:
				# Persist the provided metadata (not the server's internal attr)
			...
			# Verify the written file parses as JSON and is non-empty to avoid replacing
			# the good metadata with a corrupt file. If verification fails, attempt
			# to restore the backup.
			try:
				with open(self.METADATA_FILE, 'r', encoding='utf-8') as vf:
					loaded = json.load(vf)
				if not loaded:
					raise ValueError('Written metadata is empty')
			except Exception as e:
				logging.critical(f"Persisted metadata appears invalid after write: {e}")
				# try to restore from backup if available
				try:
					bak_files = [p for p in [self.METADATA_FILE + ".bak." + time.strftime("%Y%m%d-%H%M%S")] if os.path.exists(p)]
					# fallback: find any .bak.* matching files
					if not bak_files:
						for fname in os.listdir(os.path.dirname(self.METADATA_FILE)):
							if fname.startswith(os.path.basename(self.METADATA_FILE) + ".bak"):
								bak_files.append(os.path.join(os.path.dirname(self.METADATA_FILE), fname))
					if bak_files:
						shutil.copy2(bak_files[-1], self.METADATA_FILE)
						logging.info(f"Restored metadata from backup {bak_files[-1]}")
				except Exception:
					pass
				return False
			...
			# fsync directory
			try:
				dirfd = os.open(os.path.dirname(self.METADATA_FILE), os.O_DIRECTORY)
				try:
					os.fsync(dirfd)
				finally:
					os.close(dirfd)
			except Exception as e:
				logging.warning(f"fsync(metadata dir) failed: {e}")
			return True
		except OSError as e:
			logging.critical(f"Failed to persist metadata to {self.METADATA_FILE}: {e}")
			# keep tmp if exists for manual recovery
			return False
		finally:
